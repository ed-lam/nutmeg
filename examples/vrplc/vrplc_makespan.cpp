...
int main(int argc, char** argv)
{
    ...
    for (int i = 0; i < N; ++i)
    {
        ...
        vars_start[i] = model.add_int_var(lb, ub, (i == N-1), name);
    }
    ...
    // Solve.
    model.minimize(vars_start[N-1], time_limit);
    ...
    if (model.get_status() == Status::Feasible || model.get_status() == Status::Optimal)
    {
        ...
        for (int i = 1; i <= R; ++i)
        {
            ...
            println("Customer {}: Start {}, Complete {}", i, start, end);
        }
    }
